...
├── .claude/
│   ├── agents/
│   │   ├── planner.md          # decompoe spec em fases
│   │   ├── coder.md            # implementa codigo
...
│   └── skills/
│       ├── phase-runner/
│       │   └── SKILL.md        # orquestra implementacao por fases
│       └── test-suite/
...
O phase-runner chama o planner, divide em fases, e para cada fase lanca 3 coders
→ 3 testers → fix loop → commit → proxima fase.
...
deterministicos, depois lanca 3 unit-testers + 1 Playwright agent em paralelo.
Gera TEST-REPORT.md com pass rate, bugs, e recomendacao.
...
### Implementacao (phase-runner)
| Agent | Papel | Tools |
|-------|-------|-------|
| `planner` | Le spec, cria IMPLEMENTATION-PLAN.md com fases | Read, Glob, Grep |
...
### Phase Runner
- `IMPLEMENTATION-PLAN.md` — plano de implementacao
- `STATUS.md` — status de cada fase
- `AUDIT-phase-N.md` — auditoria por fase

### Test Suite
- `TEST-IMPLEMENTATION-PLAN.md` — test cases deterministicos (UT-xxx, CT-xxx, E2E-xxx)
- `TEST-RESULTS-unit.md` — resultados unit/component
- `TEST-RESULTS-e2e.md` — resultados Playwright
- `TEST-REPORT.md` — relatorio consolidado
...
### Mudar numero de agents

Edite o SKILL.md de cada orquestrador. O padrao e 3 coders + 3 testers
(phase-runner) e 3 unit-testers + 1 e2e-tester (test-suite).

### Mudar frameworks de teste

O default e Vitest (unit/component) + Playwright (E2E). Para mudar:
- Edite `unit-tester.md` — troque referencias a Vitest por Jest/Mocha/etc
